...
let rec populate_from cfg vaddr depth =
  ...
  let int_addr = Virtual_address.to_bigint vaddr |> Bigint.int_of_big_int in
  let stop = Basic_types.Int.Set.(Sse_options.(
      mem int_addr (GoalAddresses.get()) || mem int_addr (AvoidAddresses.get())
    )) in
  if G.mem_vertex_a cfg root_addr <> None then () else
    ...
    Dhunk.iteri
      ~f:(fun index dba_instr ->
          ...
          if depth - index >= 0 then begin
            ...
            let targets = match instr_succ addr dba_instr with None -> [] | Some x -> x in
            List.iter (function
                | Dba.JInner id ->
                  if depth - id >= 0 then
